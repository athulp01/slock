...
int is_proc_running(const char *proc_cmd) {
  regex_t number;
  regex_t name;
  regcomp(&number, "^[0-9]\\+$", 0);
  regcomp(&name, proc_cmd, 0);
  chdir("/proc");
...
int main(int argc, char **argv) {
  ...
  for (int prog = 0; prog < sizeof programs / sizeof *programs; prog++) {
    if (is_proc_running(programs[prog])) {
      die("running");
      return 0;
    }
  }
